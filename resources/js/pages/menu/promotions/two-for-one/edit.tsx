...
function groupPromotionItems(items: PromotionItem[], products: Product[]): LocalTwoForOneItem[] {
    ...
    items.forEach((item, index) => {
        ...
        const groupKey = `${item.variant_id || 'null'}`;

        const relatedItems = items
            .map((it, idx) => ({ item: it, index: idx }))
            .filter(
                ({ item: it, index: idx }) => !processed.has(idx) && it.product_id && `${it.variant_id || 'null'}` === groupKey
            );
...
export default function EditTwoForOnePromotion({ promotion, products, categories }: EditPromotionPageProps) {
    ...
    const [localItems, setLocalItems] = useState<LocalTwoForOneItem[]>(() => groupPromotionItems(promotion.items, products));
