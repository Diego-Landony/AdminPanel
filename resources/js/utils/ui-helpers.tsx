import { Calendar, Clock, CreditCard, Mail, MapPin, Phone, Star } from 'lucide-react';
...
export const formatCardWithIcon = (cardNumber: string, className: string = 'text-sm') => (
    <div className={`flex items-center gap-2 ${className}`}>
        ...
        <code className="truncate rounded bg-muted px-2 py-1 font-mono text-sm">{cardNumber}</code>
    </div>
);
 ...
 */
export const formatNumericWithLabel = (value: number | string, label: string, className: string = 'text-sm') => (
 ...
 */
export const formatStatusWithCount = (status: string, count: number, icon: React.ReactNode, className: string = '') => (
    <div className={`flex items-center gap-2 ${className}`}>
        {icon}
        <div>
...
export const TruncatedTextWithTooltip = ({
    ...
    return (
        <span className={`${className} ${shouldTruncate ? 'cursor-help' : ''}`} title={shouldTruncate ? text : undefined}>
            {truncatedText}
        </span>
    );
};
