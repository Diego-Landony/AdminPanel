...
class OrderStatusResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'current_status_label' => self::getStatusLabel($this->status, $this->service_type),
            ...
            'history' => $this->whenLoaded('statusHistory', fn () => $this->statusHistory->map(fn ($h) => [
                'status' => $h->new_status,
                'status_label' => self::getStatusLabel($h->new_status, $this->service_type),
     ...
     */
    public static function getStatusLabel(?string $status, ?string $serviceType = null): ?string
    {
        if ($status === 'ready') {
